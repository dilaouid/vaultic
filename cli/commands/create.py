...
def create_new_vault(
    name: Optional[str] = typer.Option(None, "--name", "-n", help="Name for the new vault"),
    linked: bool = typer.Option(False, "--linked", "-l", help="Link vault to main passphrase"),
    independent: bool = typer.Option(False, "--independent", "-i", help="Use a separate passphrase for this vault"),
):
    """
    ...
    try:
        if linked and independent:
            print("[red]❌ Error:[/red] You can't use both --linked and --independent simultaneously.")
            raise typer.Exit(code=1)

        if not linked and not independent:
            print("[red]❌ Error:[/red] You must specify either --linked or --independent.")
            raise typer.Exit(code=1)
