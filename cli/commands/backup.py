...
def backup_file(
    ...
    vault_id: Optional[str] = typer.Option(None, "--vault", "-v", help="Specific vault to use"),
    provider: Optional[str] = typer.Option(None, "--provider", "-p", help="Override storage provider"),
    passphrase: Optional[str] = typer.Option(None, "--passphrase", help="Vault passphrase (will prompt if not provided)")
):
    """
    ...
    try:
        ...
        print(f"[blue]🔐 Backing up file to vault {selected_vault_id}:[/blue] {filename}")
...
def backup_dir(
    ...
    provider: Optional[str] = typer.Option(None, "--provider", "-p", help="Override storage provider"),
    passphrase: Optional[str] = typer.Option(None, "--passphrase", help="Vault passphrase (will prompt if not provided)"),
    exclude: List[str] = typer.Option([], "--exclude", "-e", help="Patterns to exclude (can be used multiple times)"),
    recursive: bool = typer.Option(True, "--recursive/--no-recursive", help="Backup subdirectories recursively")
):
    """
    ...
    try:
        ...
        print(f"[blue]🔐 Backing up {total_files} files to vault {selected_vault_id}...[/blue]")
        ...
        if successful < total_files:
            print(f"[yellow]⚠️ {total_files - successful} files failed to process[/yellow]")
            raise typer.Exit(code=1)
