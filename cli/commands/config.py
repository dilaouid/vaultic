...
def show_config():
    ...
    if Config.PROVIDER == "google_drive":
        ...
        print(f"[green]Client ID:[/green] {'Set' if Config.GOOGLE_CLIENT_ID else 'Not Set'}")
        print(f"[green]Client Secret:[/green] {'Set' if Config.GOOGLE_CLIENT_SECRET else 'Not Set'}")
        print(f"[green]Refresh Token:[/green] {'Set' if Config.GOOGLE_REFRESH_TOKEN else 'Not Set'}")
        ...
        print(f"[green]Account ID:[/green] {'Set' if Config.B2_ACCOUNT_ID else 'Not Set'}")
        print(f"[green]Application Key:[/green] {'Set' if Config.B2_APPLICATION_KEY else 'Not Set'}")
        print(f"[green]Bucket Name:[/green] {Config.B2_BUCKET_NAME or 'Not Set'}")

@app.command("set")
def set_config(
    key: str = typer.Argument(..., help="Configuration key to set (e.g., PROVIDER, VAULTIC_BACKUP_DIR)"),
